...
// Define all the necessary GPIOs required for our application.
...
#define time	2000
#define Num_pas 500 // number of steps
...
void Gpio_Config()
{
	...
	EDIS; 									// This is needed to disable writing to EALLOW protected registers
}
...
void main(void)
{
	...
	while(1)
	{
		for(p = 0; p < Num_pas; p++){ 		// 500 steps in the positive direction
			step_direction(0);
			DELAY_US(time);				// time between each step
		}
		for(n = 0; n < Num_pas; n++){      // 500 steps in the negative direction
			step_direction(1);
			DELAY_US(time);				// time between each step
		}
	}
